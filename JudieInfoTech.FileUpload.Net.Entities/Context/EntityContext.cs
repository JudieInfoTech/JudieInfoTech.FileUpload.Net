      ...
      using (var resourceStream = assembly.GetManifestResourceStream(fileName))
      {
        using (var reader = new StreamReader(resourceStream, Encoding.UTF8))
        {
          var document = reader.ReadToEnd();
            ...
            var uploadedFiles = JsonConvert.DeserializeObject<List<UploadedFile>>(document);
            foreach (var uploadedFile in uploadedFiles)
            {
                Add(uploadedFile);
                Entry(uploadedFile).State = EntityState.Added;

                var ra = SaveChanges();

                Entry(uploadedFile).State = EntityState.Detached;
            }
        }
      }
    }
